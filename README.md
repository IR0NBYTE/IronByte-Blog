# IronByte Blog

A collection of technical writeups and articles on Android reverse engineering, ARM architecture, CTF challenges, and competitive programming.

## Posts

### Android Reverse Engineering

A multi-part series covering Android internals from the ground up -- framework architecture, runtime execution, APK structure, native code, and ARM assembly.

- [Chapter 1: Android Fundamentals](posts/android-reverse-engineering/01-android-fundamentals.md)
- [Chapter 2: Unlocking Android Mysteries](posts/android-reverse-engineering/02-unlocking-android-mysteries.md)
- [Chapter 3: Smashing The APK](posts/android-reverse-engineering/03-smashing-the-apk.md)
- [Chapter 4: Fortress Of The Unbreakable](posts/android-reverse-engineering/04-fortress-of-the-unbreakable.md)
- [Chapter 5: ARM's Intricacies](posts/android-reverse-engineering/05-arm-intricacies.md)

### CTF Writeups

Solutions and walkthroughs from various CTF competitions.

- [404 CTF 2022 - Sans Protection](posts/ctf-writeups/404ctf-2022-sans-protection.md) -- buffer overflow exploitation
- [SEECTF 2022 - 4mat](posts/ctf-writeups/seectf-2022-4mat.md) -- format string vulnerability
- [CSAW 2022 - Anya](posts/ctf-writeups/csaw-2022-anya.md) -- Unity game reverse engineering
- [CSAW 2022 - ezMaze](posts/ctf-writeups/csaw-2022-ezmaze.md) -- PyTorch model analysis
- [Securinets Jingle Bell 2022](posts/ctf-writeups/securinets-jingle-bell-2022.md) -- 6 crackme challenges
- [The Verifier - YinkoShield CTF](posts/ctf-writeups/the-verifier.md) -- Android reverse engineering with anti-Frida, anti-emulator, anti-repackaging, Merkle integrity, and a Prüfer-tree key derivation

### Algorithms

- [Sorting & Searching](posts/algorithms/sorting-and-searching.md) -- classic competitive programming problems with C++ solutions